(function() {
  ...
  window.initRecapCharts = function() {
    // 主力资金流向
    var flowEl = document.getElementById('chart-capital-flow');
    if (flowEl && !flowEl.getAttribute('data-init')) {
      flowEl.setAttribute('data-init', '1');
      var flowChart = echarts.init(flowEl, null, { renderer: 'svg' });
      flowChart.setOption({
        animation: false,
        tooltip: { trigger: 'axis', appendToBody: true, formatter: '{b}<br/>净流入: {c}亿元' },
        grid: { left: 80, right: 30, top: 20, bottom: 30 },
        xAxis: {
          type: 'value',
          axisLine: { lineStyle: { color: rule } },
          axisLabel: { color: muted, fontSize: 10, formatter: '{value}亿' },
          splitLine: { lineStyle: { color: rule, type: 'dashed' } }
        },
        yAxis: {
          type: 'category',
          data: ['电子(出)','半导体(出)','医药生物(入)','通信(入)','有色金属(入)','银行(入)','光通信(入)','创新药(入)'],
          axisLine: { lineStyle: { color: rule } },
          axisLabel: { color: muted, fontSize: 11 }
        },
        series: [{
          type: 'bar',
          data: [
            { value: -44, itemStyle: { color: down } },
            { value: -70, itemStyle: { color: down } },
            { value: 208, itemStyle: { color: up } },
            { value: 99, itemStyle: { color: up } },
            { value: 33, itemStyle: { color: up } },
            { value: 20, itemStyle: { color: up } },
            { value: 90, itemStyle: { color: up } },
            { value: 77, itemStyle: { color: up } }
          ],
          barWidth: '50%',
          label: { show: true, position: 'right', color: ink, fontSize: 11, formatter: '{c}亿' }
        }]
      });
      window.addEventListener('resize', function() { flowChart.resize(); });
    }

    // 涨跌停统计
    var limitEl = document.getElementById('chart-limit-stats');
    if (limitEl && !limitEl.getAttribute('data-init')) {
      limitEl.setAttribute('data-init', '1');
      var limitChart = echarts.init(limitEl, null, { renderer: 'svg' });
      limitChart.setOption({
        animation: false,
        tooltip: { trigger: 'axis', appendToBody: true },
        legend: { data: ['涨停','跌停','上涨','下跌'], textStyle: { color: muted, fontSize: 11 }, top: 0 },
        grid: { left: 50, right: 20, top: 30, bottom: 30 },
        xAxis: {
          type: 'category',
          data: ['8月19日','8月20日','8月21日'],
          axisLine: { lineStyle: { color: rule } },
          axisLabel: { color: muted, fontSize: 11 }
        },
        yAxis: {
        ...
        series: [
          { name: '涨停', type: 'bar', data: [15, 65, 55], itemStyle: { color: up }, barWidth: '15%' },
          { name: '跌停', type: 'bar', data: [280, 5, 10], itemStyle: { color: down }, barWidth: '15%' },
          { name: '上涨', type: 'line', data: [350, 4100, 2800], smooth: true, symbol: 'circle', symbolSize: 6, itemStyle: { color: accent }, lineStyle: { width: 2 } },
          { name: '下跌', type: 'line', data: [4750, 900, 2100], smooth: true, symbol: 'circle', symbolSize: 6, itemStyle: { color: accent2 }, lineStyle: { width: 2 } }
        ]
      });
      window.addEventListener('resize', function() { limitChart.resize(); });
    }
  };
