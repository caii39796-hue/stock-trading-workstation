  ...
  function renderUSPrediction() {
    ...
    if (total > 0.5) {
      scenarios.push(['情景A · 延续上行（概率偏高）', 'var(--up)', 'rgba(239,68,68,0.06)',
        '收盘结构强（多数指数收于日内高位），隔夜动能有望延续到下一交易日。若下一交易日费半高开且半小时站稳，A股半导体链（芯片ETF、烽火通信、存储自选股）开盘大概率高开联动，可按既定做T计划在冲高时先兑现部分。']);
    } else if (total < -0.5) {
      scenarios.push(['情景A · 承压下行（概率偏高）', 'var(--down)', 'rgba(34,197,94,0.06)',
        '收盘结构弱（尾盘抛压、收盘位置低），隔夜美股走弱将压制下一交易日A股科技股开盘情绪。芯片ETF、烽火通信若低开跌破防守位，按止损纪律执行不犹豫；低开过深反而可能是错杀低吸机会，重点看费半期货与旭光电子的相对强弱。']);
    } else {
  ...
  function renderGlobal() {
    ...
    html += '<div class="card" style="margin-bottom:24px;">'
      ...
      + chainRow('AI算力/光模块链', [['英伟达', nvda.pct], ['费半', sox.pct], ['光模块组', optAvg !== null ? optAvg : 0]], function(avg) {
        return avg > 0.5 ? 'AI算力链全球共振偏多，光模块（中际旭创/新易盛/天孚通信）与通信（烽火通信/沪电股份）次日有望联动，持仓的烽火通信关注反弹力度'
          : (avg < -0.5 ? 'AI算力链隔夜走弱，A股光模块/通信链开盘承压，烽火通信若低开不破防守位可持有观察，破位按纪律执行'
    ...
    html += '<div class="card" style="margin-bottom:24px;">'
      ...
      + holdRow('烽火通信', '通信/AI硬件 ← AI芯片组（英伟达/博通/AMD）+ 费半', aiAvg !== null ? aiAvg : 0,
        aiAvg !== null && aiAvg > 0.5 ? 'AI硬件链偏暖，反弹看41.5-42.0压力，接近可高抛做T' : 'AI链指引中性偏弱，38.5-39.0支撑区纪律持有，破37.5止损')
      + holdRow('芯片ETF', '费半 + 三星电子 + SK海力士', (sox.pct + samsung.pct + hynix.pct) / 3,
        sox.pct + samsung.pct + hynix.pct > 0.5 ? '全球半导体共振偏多，ETF反弹可期，但-16.4%深套仓位以时间换空间' : '全球半导体方向不明，ETF继续持有等待行业右侧信号')
    ...
    focus.push('<strong>4. 账户纪律：</strong>当前仓位65.0%、可用资金8,171元——仓位健康攻守兼备；外围利好可用于「做T降成本」或逢低分批布局，适度分散配置。');
